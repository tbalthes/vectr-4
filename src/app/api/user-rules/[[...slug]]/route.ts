...
function buildFastApiUrl(pathname: string, searchParams: URLSearchParams) {
  ...
  const qs = searchParams.toString();
  return qs
    ? `${FASTAPI_BASE}${base}/${path}?${qs}`
    : `${FASTAPI_BASE}${base}/${path}`;
}
...
export async function GET(request: NextRequest) {
  const fastApiUrl = buildFastApiUrl(
...
export async function POST(request: NextRequest) {
  ...
  if (rest[0] === "reorder") {
    ...
    if (Array.isArray(body)) payload = body;
      ...
      body &&
      typeof body === "object" &&
      (body as Record<string, unknown>).hasOwnProperty("rule_ids")
    ) {
      payload = (body as Record<string, unknown>)["rule_ids"];
    } else if (body && Array.isArray(body)) payload = body;

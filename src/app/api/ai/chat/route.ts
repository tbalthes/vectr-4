...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!genAI) {
      console.error(
        "Gemini client not initialized: missing or invalid GEMINI_API_KEY"
      );
    ...
    const {
      ...
      checkPlannerQuota,
      KEY_LABEL_MAP,
    } = await import("@/lib/analytics/validate-planner");
    ...
    function extractJsonLike(input: string) {
      ...
      try {
        ...
        const fenceMatch = input.match(/```(?:json|js|ts)?([\s\S]*?)```/i);
    ...
    if (!plannerText) console.log("Planner returned empty text");
    console.log("Planner raw output (snippet):", plannerText ? plannerText.slice(0, 400) : "<empty>");
    ...
    try {
      ...
      if (hasAggregator) {
        ...
        const existingRanges = new Set(requests.filter((r) => r.endpoint === 'aggregator').map((r) => String(r.params?.range)));
      ...
      requests = requests.filter((r) => {
        ...
        if (typeof paramsCopy.range === 'string') paramsCopy.range = String(paramsCopy.range).toUpperCase();
    ...
    if (requests.length === 0) {
      try {
        console.log("Planner returned no valid requests, retrying with a stricter JSON-only prompt...");
        const fenceOpen = "```json";
        const fenceClose = "```";
    ...
    if (!quota.allowed) {
      ...
      for (const r of requests) {
        ...
        fetchSummary.push({ label, endpoint: r.endpoint, params: r.params || {}, status: "skipped", reason: "planner_quota_exceeded" });
      }
    } else {
      // If no authentication information was forwarded with the AI request, skip server-side analytics
      if (!hasAuth) {
        console.warn("No auth forwarded to /api/ai/chat - skipping analytics fetches to avoid 401s");
        for (const r of requests) {
          const label = KEY_LABEL_MAP[r.endpoint] || String(r.endpoint).charAt(0).toUpperCase();
          fetchSummary.push({ label, endpoint: r.endpoint, params: r.params || {}, status: "skipped", reason: "no_auth_forwarded" });
        }
      } else {
        // Perform analytics fetches while forwarding available auth info
        for (const reqItem of requests) {
          try {
            const label = KEY_LABEL_MAP[reqItem.endpoint] || String(reqItem.endpoint).charAt(0).toUpperCase();
            const params = new URLSearchParams();
            for (const [k, v] of Object.entries(reqItem.params || {})) params.set(k, String(v));
            ...
            const hostHeader = (req.headers && typeof req.headers.get === 'function') ? req.headers.get('host') : null;
            const protoHeader = (req.headers && typeof req.headers.get === 'function') ? req.headers.get('x-forwarded-proto') : null;
            ...
            if (reqItem.endpoint === "aggregator") {
              ...
              fetchSummary.push({ label, endpoint: reqItem.endpoint, params: reqItem.params || {}, status: "failed", reason: "unsupported_endpoint" });
              continue;
            }

            if (!res) {
              fetchSummary.push({ label, endpoint: reqItem.endpoint, params: reqItem.params || {}, status: "failed", reason: "no_response" });
              continue;
            }

            if (!res.ok) {
              const bodyText = await res.text().catch(() => "");
              fetchSummary.push({ label, endpoint: reqItem.endpoint, params: reqItem.params || {}, status: "failed", reason: `http_${res.status}:${bodyText.slice(0,200)}` });
              continue;
            }
            ...
            fetchSummary.push({ label, endpoint: reqItem.endpoint, params: reqItem.params || {}, status: "ok" });
          } catch (err) {
            console.error("Error fetching analytics for request", reqItem, err);
            fetchSummary.push({ label: KEY_LABEL_MAP[reqItem.endpoint] || reqItem.endpoint, endpoint: reqItem.endpoint, params: reqItem.params || {}, status: "failed", reason: err instanceof Error ? err.message : String(err) });
          }
        }
      }
    }
      ...
      async function looksNarrow(value: unknown) {
        ...
        if (typeof value === 'object') {
          ...
          for (const c of candidates) {
            if (Array.isArray(v[c])) return (v[c] as unknown[]).length <= 1;
          }
          // fallback: check if object has a single top-level key whose value is small
          const keys = Object.keys(v);
          if (keys.length === 1) {
            const single = v[keys[0]];
            if (Array.isArray(single)) return single.length <= 1;
          }
        }
        return false;
      }
      ...
      for (const entry of fetchSummary.filter((f) => f.status === 'ok')) {
        try {
          const labelKey = `${entry.label}:${new URLSearchParams(entry.params as Record<string,string>).toString()}`;
          const current = gatheredData[labelKey];
          if (await looksNarrow(current)) {
            // attempt broader ranges
            for (const r of fallbackRanges) {
              try {
                const params = new URLSearchParams();
                for (const [k, v] of Object.entries(entry.params || {})) params.set(k, String(v));
                params.set('range', r);
                const urlBase = process.env.NEXT_PUBLIC_BASE_URL || `${(req.headers.get?.('x-forwarded-proto')||'http')}://${req.headers.get?.('host') || `localhost:${process.env.PORT||3000}`}`;
                const url = `${urlBase}/api/analytics/${entry.endpoint}?${params.toString()}`;
                const res = await fetch(url, { headers: { cookie: incomingCookies }, credentials: 'include' as RequestCredentials });
                if (!res.ok) continue;
                const json = await res.json().catch(() => null);
                if (!json) continue;
                // if this new result looks richer, replace
                if (!(await looksNarrow(json))) {
                  gatheredData[`${entry.label}:${params.toString()}`] = json;
                  fetchSummary.push({ label: entry.label, endpoint: entry.endpoint, params: Object.fromEntries(params.entries()), status: 'ok', reason: `fallback_range_${r}` });
  ...
  type AggRow = { income?: number | string; spending?: number | string; tx_count?: number | string; bucket?: string };
  type AggResp = { data?: AggRow[]; metadata?: { startDate?: string; endDate?: string } };
  ...
  function summarizeCategories(obj: unknown) {
    try {
      ...
      const arr = (o && ((o.categories as unknown) || (o.data as unknown))) || null;
      ...
      const normalized: { name: string; amount: number }[] = (arr as unknown[])
        .map((c) => {
          ...
          if (!it) return null;
          if (it.category || it.name) return { name: String(it.category ?? it.name), amount: Number(it.amount ?? it.total ?? it.spending ?? 0) };
          if (it.name && it.value) return { name: String(it.name), amount: Number(it.value) };
          return null;
        })
        .filter(Boolean) as { name: string; amount: number }[];
      normalized.sort((a, b) => b.amount - a.amount);
      const top = normalized.slice(0, 5).map((t) => ({ name: t.name, amount: +t.amount.toFixed(2) }));
  ...
  const agg = summarizeAggregator(val as unknown as unknown);
      if (agg) {
        dataSummary += `\n- ${label} (aggregator): ${agg.start ?? ''} to ${agg.end ?? ''} (${agg.days} days) — totalIncome=$${agg.totalIncome.toLocaleString()}, totalSpending=$${agg.totalSpending.toLocaleString()}, transactions=${agg.totalTx}, avgMonthlyIncome=$${agg.avgMonthlyIncome.toLocaleString()}, avgMonthlySpending=$${agg.avgMonthlySpending.toLocaleString()}.`;
        continue;
      }
      ...
      if (cats) {
        dataSummary += `\n- ${label} (categories): top categories: ${cats.map((c) => `${c.name} ($${c.amount.toLocaleString()})`).join(', ')}.`;
        continue;
      }

...
export interface InfiniteTransactionsResult {
  ...
  error: unknown;
  size: number;
  setSize: (size: number | ((_size: number) => number)) => Promise<unknown> | void;
  meta?: {
    totalItems: number;
    currentPage: number;
    pageSize: number;
...
export function useInfiniteTransactions(
  ...
  const groupedTransactions = flatTransactions.reduce(
    (groups, transaction) => {
  ...
  const mapUpdatedFields = useCallback((updated: Record<string, unknown>) => {
    ...
    if (updated.amount !== undefined) {
      mapped.amount = updated.amount;
    }
    if (updated.balance !== undefined) {
      mapped.balance = updated.balance;
    }
    if (updated.date !== undefined) {
      mapped.date = updated.date;
    }
  ...
  const updateTransactionOptimistic = useCallback(
    (updated: Record<string, unknown>) => {
      ...
      void mutate((pages: ApiResponse[] | undefined) => {
        if (!pages) {
          return pages;
        }
        ...
        return pages.map((page) => {
          ...
          return {
            ...page,
            data: page.data.map((t) => (t.id === String(updated.id) ? { ...t, ...mapped } : t)),
          } as ApiResponse;
        });
      }, false);
    },

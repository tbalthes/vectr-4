...
export const MessageHeader: React.FC<MessageHeaderProps> = ({ from, timestamp, className }) => {
...
const Message: React.FC<MessageProps> = ({ from, timestamp, className, children }) => {
    ...
    <div className={cn('group relative mb-6', className)}>
      <div className={cn('flex gap-3', isUser ? 'flex-row-reverse' : 'flex-row')}>
        ...
        <div className={cn('flex-1 min-w-0 max-w-[80%]', isUser ? 'items-end' : 'items-start')}>
          ...
          <div
            className={cn('flex items-center gap-2 mb-1', isUser ? 'justify-end' : 'justify-start')}
          >
          ...
          <div
            className={cn(
              ...
              isUser ? 'bg-blue-500 text-white ml-8' : 'bg-secondary text-foreground mr-8',
            )}
          >
            {children}
          </div>
...
const MessageContent: React.FC<MessageContentProps> = ({ className, children }) => {

...
export default function SpendingCalendarHeatmap({
  ...
  const colorFg = safeColor(
    css.getPropertyValue("--muted-foreground"),
    "#a3a3a3"
  );
  const colorMid = safeColor(
    css.getPropertyValue("--primary-foreground"),
    "#94a3b8"
  );
  ...
  const daily = useMemo(() => {
    ...
    return Array.from(map.entries()).map(
      ([d, amount]) => [d, amount] as [string, number]
    );
  }, [data, transactions]);
  ...
  const rangeParam = useMemo((): string | [string, string] => {
    ...
    if (
      !yearOrRange ||
      (typeof yearOrRange === "string" && yearOrRange === "last12")
    ) {
      const end = new Date();
    ...
    // If tuple already
    if (Array.isArray(yearOrRange)) return yearOrRange;
    ...
    if (/^\d{4}$/.test(yearOrRange)) return yearOrRange;
  ...
  function percentileRanks(values: number[]) {
    if (!values.length) return new Map<number, number>();
  ...
  const processedData = useMemo(() => {
    ...
    if (mode === "percentile") {
      return daily.map(
        ([d, a]) => [d, percentMap.get(a) ?? 0, a] as [string, number, number]
      );
    }
    if (mode === "log") {
      return daily.map(
        ([d, a]) =>
          [d, a > 0 ? Math.log10(a) : 0, a] as [string, number, number]
      );
    }
  ...
  const visualMax =
    mode === "percentile"
      ? 100
      : Math.max(...processedData.map(([, v]) => v as number), 1);
  ...
  const option: echarts.EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (
        ...
        const original: number = Number(params.value?.[2] ?? 0);

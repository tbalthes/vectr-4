...
export function RuleBuilder({ rule, userId, onSave, onCancel, className }: RuleBuilderProps) {
  ...
  const handleFieldChange = useCallback(
    (field: MatchField) => {
      ...
      const isValidOperator = operators.some((op) => op.value === currentOperator);

      setFormData((prev) => ({
        ...prev,
        match_field: field,
        match_operator: isValidOperator ? currentOperator : (operators[0].value as MatchOperator),
      }));
    },
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/user-rules/preview?user_id=${userId}`, {
        method: 'POST',
        headers: {
      ...
      setPreviewError(error instanceof Error ? error.message : 'Failed to preview rule');
    } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const url = rule ? `/api/user-rules/${rule.id}?user_id=${userId}` : `/api/user-rules`;
  ...
  return (
    <div className={`space-y-6 ${className || ''}`}>
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">{rule ? 'Edit Rule' : 'Create New Rule'}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => handleFieldChange(value as MatchField)}
              >
                <SelectTrigger className={errors.match_field ? 'border-red-300' : ''}>
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className={errors.match_operator ? 'border-red-300' : ''}>
            ...
            <div className="space-y-2">
              ...
              {formData.match_operator === 'regex' && (
                <p className="text-xs text-gray-500">
                  Use JavaScript regex syntax. Example: ^AMAZON.*$ for transactions starting with
                  AMAZON
                </p>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-3 pt-4 border-t">
            ...
            {onCancel && (
              <Button onClick={onCancel} variant="ghost" className="flex items-center space-x-2">
                <X className="h-4 w-4" />
                <span>Cancel</span>
              </Button>
            )}
      ...
      {/* Preview Panel */}
      {showPreview && (
        <RulePreviewPanel preview={preview} loading={isPreviewLoading} error={previewError} />
      )}
    </div>
  );
}

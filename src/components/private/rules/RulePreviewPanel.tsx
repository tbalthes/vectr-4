...
export function RulePreviewPanel({ preview, loading, error }: RulePreviewPanelProps) {
  if (loading) {
    return (
      <Card>
      ...
      <Card className="border-gray-200 bg-gray-50">
        <CardContent className="p-6">
          <div className="text-center text-gray-500">
            ...
            <p className="text-sm">
              Click &ldquo;Preview Rule&rdquo; to test against your recent transactions
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm font-medium text-blue-900">{rule_summary}</p>
            </div>

            {/* Statistics */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-900">{total_transactions_checked}</div>
                <div className="text-sm text-gray-600">Transactions Checked</div>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600">{would_override_count}</div>
                <div className="text-sm text-gray-600">Would Override</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {total_transactions_checked > 0
                    ? Math.round((matching_transactions.length / total_transactions_checked) * 100)
                    : 0}
                  %
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Showing {matching_transactions.length} sample transactions that match this rule
            </p>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {matching_transactions.map((transaction) => (
                <TransactionMatchCard key={transaction.transaction_id} transaction={transaction} />
              ))}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-gray-200">
          <CardContent className="p-6 text-center">
            <div className="text-gray-500">
              <CheckCircle className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p className="text-sm">No transactions match this rule in the recent sample.</p>
              <p className="text-xs text-gray-400 mt-1">
                Try adjusting the rule criteria or check if you have recent transactions.
              </p>
            </div>
          </CardContent>
        </Card>
...
function TransactionMatchCard({ transaction }: TransactionMatchCardProps) {
  const isOverride = transaction.current_category_name !== transaction.matched_category_name;
        ...
        <div className="flex items-center space-x-4 mt-1">
          ...
          <div className="flex items-center space-x-1 text-xs">
            {isOverride ? (
              ...
              <span className="text-gray-600">{transaction.matched_category_name}</span>
            )}
          </div>
        </div>
      </div>

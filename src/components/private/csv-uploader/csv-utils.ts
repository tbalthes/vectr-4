...
export function getColumnSuggestions(headers: string[]): Partial<ColumnMapping> {
  ...
  headers.forEach((header) => {
    ...
    // Date patterns
    if (!suggestions.date && (headerLower.includes('date') || headerLower.includes('time'))) {
...
export function combineAmounts(amounts: Record<string, string>, _headers: string[]): number {
  let totalAmount = 0;
  ...
  Object.entries(amounts).forEach(([columnName, value]) => {
    ...
    if (parsedAmount === 0 && value.trim() === '') {
      return;
    } // Skip empty values

    // Determine if this is a credit or debit column
    if (
      columnLower.includes('debit') ||
      columnLower.includes('withdrawal') ||
      columnLower.includes('payment') ||
      columnLower.includes('outgoing') ||
      columnLower.includes('expense') ||
      columnLower.includes('out')
    ) {
      ...
      columnLower.includes('incoming') ||
      columnLower.includes('received') ||
      columnLower.includes('income') ||
      columnLower.includes('in')
    ) {

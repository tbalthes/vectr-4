...
import { ChevronLeft, Settings, AlertCircle, CheckCircle, X } from 'lucide-react';
...
export function ColumnMappingStep({
  ...
  const [transactionNumber, setTransactionNumber] = useState(mapping.transactionNumber || '');
  const [description, setDescription] = useState(mapping.description || '');
  const [date, setDate] = useState(mapping.date || '');
  const [amountColumns, setAmountColumns] = useState<string[]>(mapping.amountColumns || []);
  const [balance, setBalance] = useState(mapping.balance || '');
  const [customFields, setCustomFields] = useState<string[]>(Object.values(mapping.customFields));
  ...
  const isValidAmountColumn = (header: string): boolean => {
    ...
    if (colIndex === -1 || !rawRows.length) {
      return true;
    }

    const sampleRows = rawRows.slice(selectedHeaderRow + 1, selectedHeaderRow + 6);
  ...
  const handleCustomFieldToggle = (header: string, checked: boolean) => {
    ...
    setTimeout(() => {
      onMappingChange({
        ...
        balance: balance,
        amountColumns: amountColumns,
        customFields: newCustomFields.reduce(
          (acc, field, index) => {
            acc[`custom_${index + 1}`] = field;
            return acc;
          },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-4">
          <h4 className="font-medium text-zinc-900 dark:text-zinc-50">Required Fields</h4>

          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <Label className="flex items-center gap-2 text-sm font-medium">
                Transaction Number <span className="text-red-500">*</span>
                {transactionNumber && <CheckCircle className="w-3 h-3 text-green-600 " />}
              </Label>
              <Select value={transactionNumber} onValueChange={handleTransactionNumberChange}>
                <SelectTrigger
            ...
            <div className="space-y-4">
              <Label className="flex items-center gap-2 text-sm font-medium">
                ...
                {description && <CheckCircle className="w-3 h-3 text-green-600" />}
              </Label>
            ...
            <div className="space-y-4">
              ...
              <Select value={date} onValueChange={handleDateChange}>
                <SelectTrigger
                  className={`${selectTriggerClasses} ${!date ? destructiveBorderClasses : ''}`}
                >
                  <SelectValue placeholder="Select column..." />
                </SelectTrigger>
                <SelectContent className="bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800">
            ...
            {hasBalance && (
              <div className="space-y-4">
                <Label className="flex items-center gap-2 text-sm font-medium">
                  ...
                  {balance && <CheckCircle className="w-3 h-3 text-green-600" />}
                </Label>
          ...
          <div className="space-y-3">
            <Label className="flex items-center gap-2 text-sm font-medium">
              ...
              {amountColumns.length > 0 && <CheckCircle className="w-3 h-3 text-green-600" />}
            </Label>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Select one or more amount columns. Only columns containing &apos;amount&apos;,
              &apos;credit&apos;, or &apos;debit&apos; with numeric data are shown.
            </p>
            ...
            {validAmountColumns.length > 0 ? (
                ...
                {validAmountColumns.map((header) => (
                  <div key={header} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleAmountColumnToggle(header, !!checked)}
                    />
                    <Label
              ...
              <Alert className="relative w-full rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-800 dark:border-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-300">
                ...
                <AlertDescription className="ml-6">
                  <strong>No valid amount columns found.</strong> Please ensure your CSV has columns
                  with &apos;amount&apos;, &apos;credit&apos;, or &apos;debit&apos; in the header
            ...
            {amountColumns.length > 0 && (
              <div className="text-xs text-zinc-500 dark:text-zinc-400">
                ...
                {amountColumns.length > 1 && ' (will be combined into single Amount)'}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h4 className="font-medium text-zinc-900 dark:text-zinc-50">Custom Fields</h4>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {customFields.map((field) => (
                  ...
                  >
                    <span className="text-sm text-zinc-800 dark:text-zinc-200">{field}</span>
                    <button
          ...
          {remainingHeaders.length > 0 && customFields.length < 2 && (
            <div>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-3">
                ...
                {2 - customFields.length !== 1 ? 's' : ''} to include in your import:
              </p>
                ...
                {remainingHeaders.map((header) => (
                  <div key={header} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleCustomFieldToggle(header, !!checked)}
                    />
                    <Label
                      htmlFor={header}
                      className={`text-sm font-normal cursor-pointer ${
                        customFields.length >= 2 ? 'text-zinc-400 dark:text-zinc-600' : ''
                      }`}
                    >
                      {header || `Column ${headers.indexOf(header) + 1}`}
                    </Label>
          ...
          {customFields.length >= 2 && remainingHeaders.length > 0 && (
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Maximum of 2 custom fields reached. Remove a field above to add a different one.
            </p>
          )}
      ...
      <div className="flex justify-between pt-6 border-t border-zinc-200 dark:border-zinc-800">
        ...
        <Button onClick={() => onComplete(mapping, selectedAccountId)} disabled={!isFormValid}>
          Continue to Preview

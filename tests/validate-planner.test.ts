import { validatePlannerRequests, checkPlannerQuota } from '../src/lib/analytics/validate-planner';
...
describe('validatePlannerRequests', () => {
  ...
  it('coerces string booleans and numbers', () => {
    const raw = { requests: [ { endpoint: 'categories', params: { namesOnly: 'true', range: 7 } } ] };
    const out = validatePlannerRequests(raw);
    expect(out.length).toBe(1);
